...
def plot_var_tiles(batch, pred, met_vars, b=0, v='TA', sea_mask=None,
                   trim_edges=False, norm=None, cmap='plasma'):
    if sea_mask is None:
    ...
    if not norm is None:
        cmap_cm = cm.get_cmap(cmap)
        norm = Normalize(np.min(pred[b,idx_m,:,:]), np.max(pred[b,idx_m,:,:]))
        im = cm.ScalarMappable(norm=norm, cmap=cmap_cm)
...
def plot_batch_tiles(batch, pred, fine_vars, met_vars,
                     ...
                     norm=None, constraints=True, cmap='plasma'):
    if sea_mask is None:
        sea_mask = np.ones(pred.shape)
        sea_mask = sea_mask[:,0:1,:,:]
...
def plot_context_and_target_preds(preds, b, batch, station_targets, var,
    ...
    if var=="WS":
        output = {}
        for vv, thisvar in enumerate(["UX", "VY"]):
            output[thisvar] = {}
            if thisvar=="UX":
            ...
            for axi, site_type in enumerate(['context', 'target']):
                yinds = station_targets[b][site_type].sub_y.values[None,...]
                xinds = station_targets[b][site_type].sub_x.values[None,...]
                site_preds = [pd.DataFrame(to_np(p)[b, vv, yinds, xinds]).melt() for p in preds]
                for kk, sp in enumerate(site_preds):
                    sp.index = station_targets[b][site_type].index
                    sp.columns = ['id', model_names[kk]]    
                joined = station_targets[b][site_type]
                for sp in site_preds:
                    joined = joined.merge(sp, on='SITE_ID', how='left')    
                joined = joined[[thisvar] + model_names].reset_index().melt(id_vars='SITE_ID')
                output[thisvar][site_type] = joined
                if joined.shape[0]>0:
                    if thisvar=="UX":
                        sns.pointplot(x='SITE_ID', y='value', hue='variable', data=joined,
                                        dodge=0.1, join=False, ax=ax1[axi]).legend()
                    else:
                        sns.pointplot(x='SITE_ID', y='value', hue='variable', data=joined,
                                        dodge=0.1, join=False, ax=ax2[axi]).legend()
                if thisvar=="UX":
                    ax1[axi].set_xticklabels(ax1[axi].get_xticklabels(), rotation=45,
                                            ha='right', rotation_mode='anchor')
                    ax1[axi].set_title(site_type)
                    for points in ax1[axi].collections: 
                        size = points.get_sizes().item()
                        new_sizes = [size * 0.33 for name in ax1[axi].get_yticklabels()]
                        points.set_sizes(new_sizes)
                else:
                    ax2[axi].set_xticklabels(ax2[axi].get_xticklabels(), rotation=45,
                        ha='right', rotation_mode='anchor')
                    ax2[axi].set_title(site_type)
                    for points in ax2[axi].collections: 
                        size = points.get_sizes().item()
                        new_sizes = [size * 0.33 for name in ax2[axi].get_yticklabels()]
                        points.set_sizes(new_sizes)
    else:    
