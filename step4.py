...
from setupdata4 import Batch
from utils2 import save_checkpoint, prepare_run, create_attention_masks
from params2 import data_pars as dps, train_pars, model_pars as mps

batch_quantiles_path = '/gws/nopw/j04/ceh_generic/netzero/downscaling/training_data/var_quantile_samples/'

def model_step(model, batch, loglikelihood,
               var=None, use_masks=False, constraint_inds=None):
...
def make_train_step(model, optimizer, loglikelihood):
    def train_step(batch, var=None, constraint_inds=None):
        model.train()
        
        # take step        
        losses = model_step(model, batch, loglikelihood,
                            var=var, constraint_inds=constraint_inds)
...
def make_val_step(model, loglikelihood):
    def val_step(batch, var=None, constraint_inds=None):
        model.eval()
...
def fit(model, optimizer, loglikelihood, var, datgen, 
    ...
    for epoch in range(curr_epoch, train_pars.max_epochs):
        ...
        for bidx in range(1, train_pars.train_len+1):
            ...
            loss_dict = train_step(batch, var=var, constraint_inds=constraint_inds)
        ...
        with torch.no_grad():
            ...
            for bidx in range(1, train_pars.val_len+1):
                ...
                loss_dict = val_step(batch, var=var, constraint_inds=constraint_inds)
                running_ls = update_running_loss(running_ls, loss_dict)
